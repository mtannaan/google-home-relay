...
type User = {id: string; username: string; password: string; name: string};
type Client = {
  id: string;
  name: string;
  clientId: string;
...
type TokenInfo = {userId: string; clientId: string};
...
server.deserializeClient((id, done) => {
  db.clients.findById(id, (error: Error, client: unknown) => {
    if (error) return done(error);
    return done(null, client);
  });
});

function issueTokens(userId: string | null, clientId: string, done: Function) {
  db.users.findById(userId, (error: Error, user: User) => {
...
server.exchange(
  oauth2orize.exchange.code((client, code, redirectUri, done) => {
    db.authorizationCodes.find(
      code,
      (error: Error, authCode: AuthorizationCode) => {
        if (error) return done(error);
...
server.exchange(
  oauth2orize.exchange.password((client, username, password, scope, done) => {
    ...
    db.clients.findByClientId(
      client.clientId,
      (error: Error, localClient: Client) => {
        if (error) return done(error);
        if (!localClient) return done(null, false);
        if (localClient.clientSecret !== client.clientSecret)
          return done(null, false);
        // Validate the user
        db.users.findByUsername(username, (error: Error, user: User) => {
          if (error) return done(error);
...
server.exchange(
  oauth2orize.exchange.refreshToken((client, refreshToken, scope, done) => {
    db.refreshTokens.find(refreshToken, (error: Error, token: TokenInfo) => {
      if (error) return done(error);
      issueTokens(
        token.userId,
        client.id,
        (err: Error, accessToken: string, refreshToken: string) => {
...
module.exports.authorization = [
  ...
  server.authorization(
    (clientId, redirectUri, done) => {
      db.clients.findByClientId(clientId, (error: Error, client: Client) => {
        if (error) return done(error);
